...
\ The TCP methods are as follows:-
\ TCPCONNECT ( ipaddr port obj -- )             \ Connect as TCP Client
\ TCPLISTEN  ( ipaddrrange port obj -- )        \ Set up as a TCP Server *
\ READ       ( addr n obj -- )                  \ Receive n bytes to addr *
\ WRITE      ( addr n obj -- )                  \ Transmit n bytes from addr *
\ CLOSE      ( obj -- )                         \ Close a TCP Client / Server
\ VARS.      ( obj -- )                         \ Display internal variables

\ * These methods 'block' until their action can complete. A future version of this code will use non-blocking
...
:noname
    ...
    cr ." Connecting to " R@ locsockaddr sockaddr. cr
...
:noname
	...
	begin
		...
		0 recv DUP 0< IF THROW THEN							( [locsock] addr nreqd nrecvd )
...
:noname
	...
	begin
		...
		0 send DUP 0< IF THROW THEN							( [remsock] addr nreqd nrecvd )
...
: (TEST1)	( -- )
	...
	0 count !												\ this variable is used to create the sawtooth
		BEGIN
...
: (TEST2)	( -- )
	z" forth1.local" gethostbyname DUP 0= IF 1 THROW THEN ->h_addr 9999  ( ipaddr port ) \ gethostbyname returns hostent OR 0 if no host found
